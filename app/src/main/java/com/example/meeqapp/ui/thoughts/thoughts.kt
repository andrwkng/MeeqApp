...
import com.example.meeqapp.ui.distortions.distortions
import kotlinx.serialization.Contextual
import kotlinx.serialization.Serializable
import java.time.LocalDate

fun newThought(): Thought {
    return ThoughtImpl(
        automaticThought = "",
        cognitiveDistortions = distortions.map { CognitiveDistortion(label = it.label, slug = it.slug) },
        challenge = "",
        alternativeThought = "",
    )
}
...
interface Thought {
    ...
    val alternativeThought: String
    var cognitiveDistortions: List<CognitiveDistortion>
    val challenge: String
    val immediateCheckup: ImmediateCheckup?
    val followUpDate: String?
    var followUpCompleted: Boolean?
